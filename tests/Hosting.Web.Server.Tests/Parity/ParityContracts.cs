...
/// <summary>
///     The role-named wrapper Futhark's "scalar collections have no shape" law (§5.8) requires for a collection of
///     strings.
/// </summary>
[DataContract]
...
public sealed record ParityReport
{
	...
	/// </summary>
	[DataMember(Order = 14)]
	public ParityStatus Status { get; init; }

	[DataMember(Order = 15)] public List<ParityReportTag> Tags { get; init; } = [];
}

/// <summary>
///     The response-side counterpart to <see cref="ParityTag" /> — a distinct type, raw (non-<c>Result</c>)
///     <see cref="Value" />.
/// </summary>
[DataContract]
