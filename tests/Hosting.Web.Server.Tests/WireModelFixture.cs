...
using Norse.Hosting.Web.Server.Tests;
using Norse.Hosting.Web.Server.Tests.Parity;
using Norse.Infrastructure.Web.Grpc;
using ProtoBuf.Meta;

[assembly: Xunit.AssemblyFixture(typeof(WireModelFixture))]
	...
	public WireModelFixture()
	{
		...
		model.EnsureRegistered(typeof(Outcome<ParityReport>), () =>
		{
			if (!model.IsDefined(typeof(Outcome<ParityReport>)))
				model.Add(typeof(Outcome<ParityReport>), applyDefaultBehaviour: false)
					.SetSurrogate(typeof(ParityReport));
		});
	}
}
